...
var (
	...
	phone3222 = regexp.MustCompile(`(\+48 *)?(\d\d\d)[ -]*(\d\d)[ -]*(\d)(\d)[ -]*(\d\d)`) // Zadanie 1
)
...
func TransformPhoneNumbers(s string) string {
	...
	s = phone3222.ReplaceAllString(s, "${2}-${3}${4}-${5}${6}")
	// Zadanie 1
	return s
}
...
func ToASCIIStems(w ASCIIWord) []ASCIIStem {
	...
	if m := reCeK.FindStringSubmatch(s); m != nil {
		return ToASCIIStemSlice(m[1] + "k") // Zadanie 2
	}
	if m := reCyK.FindStringSubmatch(s); m != nil {
		return ToASCIIStemSlice(m[1]+"c", m[1]+"k")
	}
	if m := reDzeG.FindStringSubmatch(s); m != nil {
		return ToASCIIStemSlice(m[1] + "g") // Zadanie 3
	}
	if m := reRzeR.FindStringSubmatch(s); m != nil {
		return ToASCIIStemSlice(m[1]+"r", m[1]+"rz")
	} // Zadanie 4
	if m := reCień.FindStringSubmatch(s); m != nil {
		return ToASCIIStemSlice(s, m[1]+"tn")
	} // Zadanie 5
...
func AbbreviateFacultyName(name string, rmStopwords bool) ASCIIStem {
	...
	var FacultyName = Comma.Split(name, -1)[0]
	var words = Word.FindAllString(FacultyName, -1)

	var abbr = ""

	for _, word := range words {
		if rmStopwords && word == "i" {
			continue
		}

		abbr += strings.ToLower(word[0:1])
	}

	return ASCIIStem(abbr)
}
